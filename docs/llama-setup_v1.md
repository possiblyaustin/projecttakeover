...
**Conversation appears to "forget" earlier turns past depth ~12** — by design as of v0.0.39 / v0.0.40. `LlamaCppModelService` caps the chat history forwarded to llama-server at `historyTurnCap` (default 12 exchanges as of v0.0.40, was 16 in v0.0.39) so prompt processing stays under the 30s `timeoutMs`. The v0.0.40 history rewrite injects a `[1][2][3]` options block (~50 tokens) onto every prior NPC message in the wire payload — same prompt budget supports ~25% fewer turn-pairs, hence the lower cap. The system prompt, `[HELPYR_STATE]` block, and reputation context are re-sent in full every turn so character coherence isn't affected — only the literal exchange-by-exchange transcript is windowed. Tune via `LlamaCppConfig.historyTurnCap` if needed; the per-turn `[Chat] LLM turn: prompt=..., kept=..., dropped=...` console log shows the live numbers.
